...
OUTPUT_FILE = "txt_files/verified_profiles.txt"
        ...
        for el in links:
            ...
            if is_verified_profile(username):
                if f"@{username}" not in existing and f"@{username}" not in verified_profiles:
                    print(Fore.GREEN + f"[YES] @{username} est certifié.")
...
def main():
    print(Fore.CYAN + "[...] Start collecting verified accs.")

    scrolls = load_scroll_count()
    print(Fore.CYAN + f"[i] Number of scrolls set : {scrolls}")
    ...
    driver.quit()
    print(Fore.CYAN + f"\n[YES] {len(verified)} news accs verified added : {OUTPUT_FILE}")
