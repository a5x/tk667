...
def get_info(username):
    ...
    try:
        ...
        print(Fore.RED + f"[×] Requête échouée pour : {username} ({e})")
        return None

    if response.status_code != 200:
        print(Fore.RED + f"[×] Échec pour : {username} (HTTP {response.status_code})")
        return None
    ...
    if not user_info:
        print(Fore.RED + f"[×] Pas de données pour : {username}")
        return None
...
if __name__ == '__main__':
    display_banner()
    input_file = 'Scripts_info_extract/profiles_with_email.txt'

    if not os.path.exists(input_file):
        print(Fore.RED + f"[×] Fichier introuvable : {input_file}")
        raise SystemExit(1)
    ...
    print(Fore.GREEN + f"\n[✓] Lignes mises à jour dans {input_file}")
